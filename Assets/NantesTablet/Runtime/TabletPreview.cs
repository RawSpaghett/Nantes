...
namespace NantesGame.Tablet
{
    public sealed class TabletPreview : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if(!hints)return;
            hints.text=tablet.HasFocus?"TAB  Lower device     1  Home     2  Scanner     SPACE  Pulse     F  Food +1     F11  Window":"TAB  Raise device     F11  Window";
            if(tablet.IsReady)context.text=tablet.Hover==0?"Home":tablet.Hover==1||tablet.Hover==3?"Open scanner":tablet.Hover==2?"Power down":tablet.Hover==4?"Send pulse":tablet.Hover==5?"Change scan range":"";
            else context.text="";
        }
        ...
        IEnumerator Check(string directory)
        {
            ...
            Vector2 screenPoint=Project(new Vector2(553,426));
            Assert(tablet.TryGetDisplayPoint(screenPoint,out var mapped) && (mapped-new Vector2(553,426)).magnitude<1,"Angled glass maps pointer correctly");
            ...
            Assert(tablet.State==TabletState.Scanner,"Pointer opens scanner");
            ...
            tablet.Activate(5);Assert(tablet.Range==50,"Range changes to fifty");
            tablet.Activate(5);Assert(tablet.Range==25,"Range returns to twenty-five");
            yield return new WaitForSecondsRealtime(2.1f);
            ...
            var hardware=tablet.transform.Find("Home key");
            screenPoint=tablet.viewCamera.WorldToScreenPoint(hardware.position);
            InputSystem.QueueStateEvent(mouse,new MouseState{position=screenPoint});yield return null;
            InputSystem.QueueStateEvent(mouse,new MouseState{position=screenPoint,buttons=1});yield return null;yield return null;
            InputSystem.QueueStateEvent(mouse,new MouseState{position=screenPoint});yield return null;
            Assert(tablet.State==TabletState.Home,"Physical home button works");
            tablet.ShowScanner();
            ...
            Assert(tablet.State==TabletState.Home,"Escape returns home");
            InputSystem.QueueStateEvent(keys,new KeyboardState(Key.DownArrow));yield return null;yield return null;
            InputSystem.QueueStateEvent(keys,new KeyboardState());yield return null;
            Assert(tablet.Hover==1,"Keyboard focus advances");
            InputSystem.QueueStateEvent(keys,new KeyboardState(Key.Enter));yield return null;yield return null;
            InputSystem.QueueStateEvent(keys,new KeyboardState());yield return null;
            Assert(tablet.State==TabletState.Scanner,"Enter activates focused scanner");
            ...
            yield return new WaitForSecondsRealtime(1.3f);Assert(tablet.State==TabletState.Home&&tablet.FoodCount==3,"Wake preserves count");
